...
class ICNetModelFactory(object):
    ...
    def build(
        ...
        out = SeparableConv2D(n_classes, 1, activation='softmax',
                     name='conv6_cls')(out_1)


        if train:
            aux_1 = SeparableConv2D(n_classes, 1, activation='softmax',
                           name='sub4_out')(aux_1)
            aux_2 = SeparableConv2D(n_classes, 1, activation='softmax',
                           name='sub24_out')(aux_2)
            model = Model(inputs=inpt, outputs=[out, aux_2, aux_1])
        else:
            out = UpSampling2D(size=(4, 4), name='out_upsample')(out)
